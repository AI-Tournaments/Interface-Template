...
- Call `window.opener.postMessage(/*Replace with your response*/, '*')` whenever after reception of a `messageEvent.data.type === 'Post'`. The prewritten variable `responseLock` will help with managing that only one respond is sent per `Post`.
